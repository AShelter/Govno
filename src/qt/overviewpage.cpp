...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeGovnoAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeGovnoAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeGovnoAmount = strAnonymizeGovnoAmount.remove(strAnonymizeGovnoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGovnoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeGovnoAmount*COIN) nMaxToAnonymize = nAnonymizeGovnoAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeGovnoAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeGovnoAmount));
        strAnonymizeGovnoAmount = strAnonymizeGovnoAmount.remove(strAnonymizeGovnoAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeGovnoAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
