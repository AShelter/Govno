...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeGovnoAmount"))
        settings.setValue("nAnonymizeGovnoAmount", 1000);
    nAnonymizeGovnoAmount = settings.value("nAnonymizeGovnoAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeGovnoAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeGovnoAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeGovnoAmount:
            return QVariant(nAnonymizeGovnoAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeGovnoAmount:
            nAnonymizeGovnoAmount = value.toInt();
            settings.setValue("nAnonymizeGovnoAmount", nAnonymizeGovnoAmount);
            emit AnonymizeGovnoAmountChanged(nAnonymizeGovnoAmount);
            break;
        default:
            break;
        }
