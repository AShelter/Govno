...
namespace {
    class CGovnoAddressVisitor : public boost::static_visitor<bool> {
    private:
        CGovnoAddress *addr;
    public:
        CGovnoAddressVisitor(CGovnoAddress *addrIn) : addr(addrIn) { }
...
bool CGovnoAddress::Set(const CKeyID &id) {
    SetData(Params().Base58Prefix(CChainParams::PUBKEY_ADDRESS), &id, 20);
    return true;
}

bool CGovnoAddress::Set(const CScriptID &id) {
    SetData(Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS), &id, 20);
    return true;
}

bool CGovnoAddress::Set(const CTxDestination &dest) {
    return boost::apply_visitor(CGovnoAddressVisitor(this), dest);
}

bool CGovnoAddress::IsValid() const {
...
CTxDestination CGovnoAddress::Get() const {
    if (!IsValid())
...
bool CGovnoAddress::GetKeyID(CKeyID &keyID) const {
...
bool CGovnoAddress::IsScript() const {
    return IsValid() && vchVersion == Params().Base58Prefix(CChainParams::SCRIPT_ADDRESS);
}

void CGovnoSecret::SetKey(const CKey& vchSecret) {
...
CKey CGovnoSecret::GetKey() {
    CKey ret;
    ret.Set(&vchData[0], &vchData[32], vchData.size() > 32 && vchData[32] == 1);
    return ret;
}

bool CGovnoSecret::IsValid() const {
    bool fExpectedFormat = vchData.size() == 32 || (vchData.size() == 33 && vchData[32] == 1);
    bool fCorrectVersion = vchVersion == Params().Base58Prefix(CChainParams::SECRET_KEY);
    return fExpectedFormat && fCorrectVersion;
}

bool CGovnoSecret::SetString(const char* pszSecret) {
    return CBase58Data::SetString(pszSecret) && IsValid();
}

bool CGovnoSecret::SetString(const std::string& strSecret) {
